...
def run_source_study(directory: Path, root: Path = engine.ROOT) -> dict:
    ...
    first['summary'] = 'Bounded historical source-study mandate; not live issuer diligence or investment approval.'
    ...
    for section in first['sections'].values():
        section['analysis'] = 'Identify the announcement, record its historical context, and request missing contractual, outcome and current market evidence. No investment recommendation is permitted.'
    state = engine.submit(directory, 'mandate', first, state['revision'], root)
    stage = state['frozen']['stages']['evidence']
    ev = request['evidence'][0]
    metric = ev['metrics'][0]
    fact = {'id': 'F1', 'kind': 'FACT', 'text': case['fact'], 'scope': metric['scope'], 'period': metric['period'],
            'evidence_ids': [ev['id']], 'metric': {'evidence_id': ev['id'], 'metric_id': metric['id'], 'value': metric['value'], 'unit': metric['unit']},
    ...
    artifact = {'schema_version': '1.0', 'run_id': state['run_id'], 'input_digest': state['input_digest'],
                ...
                'gaps': case['gaps'], 'issues': [{'id': 'MISSING-DILIGENCE', 'severity': 'MATERIAL', 'status': 'OPEN', 'description': 'Historical announcement does not supply the full evidence needed for investment diligence.'}],
                'confidence': {'value': 0.8, 'basis': 'Subjective confidence in the bounded announcement extraction only, not current financial attractiveness, legal validity or outcomes.'}}
